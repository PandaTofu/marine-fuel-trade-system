...
import {AccountField,cash,CommandErrors,downloadOrderDocument,MoneyInput,ReferenceInput,Status,today,useCommand,useResource} from './shared';
...
export function OrderEditor({order,onClose,onSaved}:{order?:Order;onClose:()=>void;onSaved:()=>void}){
 ...
 const totals=lineAmounts.reduce<{orderedMin:bigint;orderedMax:bigint;actual:bigint;sale:bigint;cost:bigint}>((result,line,index)=>({orderedMin:result.orderedMin+scaled(lines[index]?.ordered_qty_min,3),orderedMax:result.orderedMax+scaled(lines[index]?.ordered_qty_max,3),actual:result.actual+scaled(lines[index]?.actual_qty,3),sale:result.sale+line.sale,cost:result.cost+line.cost}),{orderedMin:0n,orderedMax:0n,actual:0n,sale:0n,cost:0n});
 ...
 const due=(days:unknown)=>{if(!values.actual_date)return '—';const date=new Date(`${values.actual_date}T00:00:00`);date.setDate(date.getDate()+Math.max(num(days)-1,0));return `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;};
 const section=(title:string,content:ReactNode)=><section className="order-section"><h3>{title}</h3>{content}</section>;
 const initialValues=order?{...order,...order.numbers,estimated_range:order.estimated_start_date&&order.estimated_end_date?[dayjs(order.estimated_start_date),dayjs(order.estimated_end_date)]:undefined}:{order_date:today(),customer_term_description:'',supplier_term_description:'',customer_term:0,supplier_term:0,commission_rate:'0',customer_fee:'0',supplier_fee:'0',berth_fee:'0',exceptional_fee:'0',customer_deposit:'0',customer_received:'0',supplier_deposit:'0',supplier_paid:'0',lines:[{oil:'',ordered_qty_min:'0',ordered_qty_max:'0',actual_qty:null,sale_price:'0',cost_price:'0'}]};
 return <Modal open width={1280} title={<div className="order-modal-title"><Space>{t(order?'biz.editOrder':'biz.createOrder')}<Language/></Space><Tooltip title={order?undefined:t('biz.saveBeforeExport')}><Button icon={<FilePdfOutlined/>} disabled={!order} onClick={()=>{if(order)void downloadOrderDocument(order.id,'contract').catch(()=>undefined);}}>{t('biz.exportContract')}</Button></Tooltip></div>} onCancel={()=>{if(!cmd.busy)onClose();}} footer={null}>
 ...
 <Form form={form} layout="vertical" initialValues={initialValues} onFinish={async values=>{
  const {estimated_range,...fields}=values;const payload={...fields,currency:'USD',estimated_start_date:estimated_range?.[0]?.format('YYYY-MM-DD')||null,estimated_end_date:estimated_range?.[1]?.format('YYYY-MM-DD')||null,actual_date:values.actual_date||null,version:order?.version,
   lines:values.lines.map((line:Record<string,unknown>)=>({oil:line.oil,ordered_qty_min:line.ordered_qty_min,ordered_qty_max:line.ordered_qty_max,actual_qty:line.actual_qty===''||line.actual_qty===undefined?null:line.actual_qty,sale_price:line.sale_price,cost_price:line.cost_price}))};
 ...
 <Form.Item label={productLabel(t('biz.orderQuantity'),'MT')} required><div className="quantity-range-inputs"><Form.Item name={[field.name,'ordered_qty_min']} noStyle rules={required}><MoneyInput compact precision={3} placeholder={t('biz.minimum')}/></Form.Item><span>–</span><Form.Item name={[field.name,'ordered_qty_max']} noStyle dependencies={[["lines",field.name,'ordered_qty_min']]} rules={[...required,({getFieldValue})=>({validator(_,value){const minimum=getFieldValue(['lines',field.name,'ordered_qty_min']);return value==null||minimum==null||scaled(value,3)>=scaled(minimum,3)?Promise.resolve():Promise.reject(new Error(t('errors.ordered_quantity_range')));}})]}><MoneyInput compact precision={3} placeholder={t('biz.maximum')}/></Form.Item></div></Form.Item>
 ...
 </div>)}<div className="order-total-row"><strong>{t('biz.total')}</strong><Tooltip title={`${quantityText(totals.orderedMin)}–${quantityText(totals.orderedMax)} MT`}><span>{compactText(quantityText(totals.orderedMin))}–{compactText(quantityText(totals.orderedMax))} MT</span></Tooltip><Tooltip title={`${quantityText(totals.actual)} MT`}><span>{compactText(quantityText(totals.actual))} MT</span></Tooltip><span/>{calculated(totals.sale)}<span/>{calculated(totals.cost,true)}<span/></div></div><Form.ErrorList errors={meta.errors}/><Button icon={<PlusOutlined/>} disabled={fields.length>=100} onClick={()=>add({oil:'',ordered_qty_min:'0',ordered_qty_max:'0',actual_qty:null,sale_price:'0',cost_price:'0'})}>{t('biz.addLine')}</Button>
 </>}</Form.List>
 </>)}
...
export function OrderDetail({order,onClose}:{order:Order;onClose:()=>void}){
 ...
 <Table rowKey={(_,index)=>String(index)} pagination={false} scroll={{x:760}} dataSource={snapshot.lines} columns={['oil','ordered_qty_min','ordered_qty_max','actual_qty','sale_price','cost_price','sale_amount','cost_amount'].map(key=>({title:t(`biz.${key}`),dataIndex:key}))}/>

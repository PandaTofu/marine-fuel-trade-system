...
export function cash(value: string | number | undefined | null) {
...
export function useCommand() {
  ...
  async function send<T = unknown>(
    ...
    setFields(undefined);
    try {
      const result = await api<T>(path, method, {
        ...data,
        request_id: pending.current.key,
      });
      pending.current = null;
      return result;
    } catch (e) {
      setError(e instanceof Error ? e.message : "errors.server_error");
...
export function CommandErrors({
  ...
  command: { error: string; fields: unknown };
}) {
  const { t } = useTranslation();
  const issues: { path: string[]; message: string }[] = [];
  const visit = (value: unknown, path: string[]) => {
    if (typeof value === "string") issues.push({ path, message: value });
    else if (value && typeof value === "object")
      Object.entries(value).forEach(([k, v]) =>
        visit(v, [
          ...path,
          ...(Array.isArray(value) && typeof v === "string" ? [] : [k]),
        ]),
      );
  };
                  ...
                  {issue.path
                    .map((key) =>
                      /^\d+$/.test(key)
                        ? String(Number(key) + 1)
                        : t(`biz.${key}`, {
                            defaultValue: t(key, { defaultValue: key }),
                          }),
                    )
                    .join(" / ")}
                  :{" "}
                  {t(`errors.${issue.message}`, {
                    defaultValue: t("biz.invalidField"),
                  })}
                </li>
              ))}
            </ul>
...
export function ReferenceInput({
  ...
  onReferenceChange?: (value: number | null) => void;
}) {
  const { t } = useTranslation();
  const r = useResource<Reference[]>("reference/?kind=" + kind);
    ...
    <div>
      <AutoComplete
        ...
        onChange={(value, option) => {
          onChange?.(value.slice(0, 160));
          const referenceId = (option as { referenceId?: number }).referenceId;
          onReferenceChange?.(referenceId ?? null);
        }}
        options={options}
      ...
      {r.error && <small>{t("biz.suggestionsUnavailable")}</small>}
    </div>
  );
}
function compactNumber(value: unknown) {
  ...
  const [suffix, divisor] =
    units.find(([, size]) => Math.abs(number) >= size) ?? units[2];
...
export function MoneyInput({
  ...
  return (
    <Tooltip title={compact && exact ? exact : null}>
      <InputNumber<string>
        stringMode
        min="0"
        precision={precision}
        value={value}
        formatter={
          compact && !focused ? (value) => compactNumber(value) : undefined
        }
  ...
  return (
    <div className="business-metrics">
      {keys.map((key) => {
        ...
        const count = key.endsWith("_count") || key === "partial_receipts";
        return (
          <Card key={key}>
            <span>{t(`biz.${key}`)}</span>
            <strong className={String(value).startsWith("-") ? "negative" : ""}>
              {count ? String(value ?? 0) : cash(value as string)}
            </strong>
          </Card>
        );
      })}
  ...
  return (
    <>
      ...
      <Select
        allowClear
        loading={r.loading}
        placeholder={t("biz.defaultAccount")}
        options={(r.data?.results || [])
          .filter((a) => a.is_active)
          .map((a) => ({
            value: a.id,
            label: `${a.name} · ${a.currency}${a.is_default ? " · " + t("biz.default") : ""}`,
          }))}
      />
    </>
  );
  ...
  return (
    <div>
      ...
      <Select
        ...
        placeholder={t("biz.defaultAccount")}
        options={(r.data?.results || [])
          .filter((a) => a.is_active && (!currency || a.currency === currency))
          .map((a) => ({
            value: a.id,
            label: `${a.name} · ${a.currency}${a.is_default ? " · " + t("biz.default") : ""}`,
          }))}
...
export function ReasonDialog({
  ...
  return (
    <Modal
      ...
      footer={null}
    >
      <Alert type="warning" message={hint} />
      ...
      >
        ...
        {clear && (
          <Form.Item
            name="confirmation"
            label={t("biz.confirmation")}
            rules={[
              {
                validator: (_, value) =>
                  value === "CLEAR"
                    ? Promise.resolve()
                    : Promise.reject(new Error(t("biz.confirmation"))),
              },
            ]}
          >
            <Input />
...
export async function downloadLedger(params: Record<string, unknown>) {
  const res = await fetch("/api/trading/ledger/export/?" + query(params), {
